      ...
      return (
        <div className="flex flex-wrap gap-1 group relative">
          {tags.slice(0, 2).map((tag) => (
            <span key={tag} className="px-2 py-0.5 text-xs rounded-md" style={{ backgroundColor: `${colors.accent.purple}20`, color: colors.accent.purple }}>
              {tag}
            </span>
          ))}
          {tags.length > 2 && (
            <span className="px-2 py-0.5 text-xs cursor-pointer relative group/tags rounded-md" style={{ backgroundColor: `${colors.accent.purple}10`, color: colors.accent.purple }}>
              +{tags.length - 2}
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="relative group">
        <button 
          className="text-xs px-3 py-1 text-white flex items-center gap-1 rounded-md transition-colors"
          style={{ backgroundColor: colors.accent.purple }}
          onMouseEnter={(e) => e.currentTarget.style.backgroundColor = colors.accent.purpleDark}
          onMouseLeave={(e) => e.currentTarget.style.backgroundColor = colors.accent.purple}
        >
        ...
        <ul className="absolute right-0 mt-1 w-36 bg-white border border-slate-200 shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 rounded-md overflow-hidden">
          <li>
            <Link
              ...
              onMouseEnter={(e) => e.currentTarget.style.backgroundColor = `${colors.accent.purple}10`}
          ...
          <li>
            <button 
              className="w-full flex items-center gap-2 px-3 py-2 text-xs text-left transition-colors"
              style={{ color: colors.accent.olive }}
              onMouseEnter={(e) => e.currentTarget.style.backgroundColor = `${colors.accent.olive}10`}
              onMouseLeave={(e) => e.currentTarget.style.backgroundColor = "transparent"}
            >
  ...
  return (
    <div className="w-full space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mt-5 animate-fade-in">
        ...
        <Link 
          to="/products/add" 
          className="inline-block px-3 py-2 text-white font-light shadow-md transition-all duration-200 rounded-lg hover:scale-105 hover:shadow-lg"
