...
export default function AppLayout() {
  ...
  const SidebarContent = (
    <>
      <div className="px-6 py-4 text-xl font-semibold border-b border-slate-200 text-slate-900">
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex">
      {/* Desktop Sidebar */}
      <aside className="fixed left-0 top-0 h-screen w-64 text-slate-900 hidden md:flex flex-col bg-white border-r border-slate-200">
        {SidebarContent}
      </aside>
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 flex">
          <aside className="h-full w-64 bg-white text-slate-900 flex flex-col shadow-xl animate-slide-in border-r border-slate-200">
    ...
    <NavLink
      ...
      className={({ isActive }) =>
        `flex items-center gap-3 px-3 py-2 text-sm font-medium transition  ${isActive
          ? "bg-[#e87427] text-white"
          : "text-slate-600 hover:bg-slate-100 hover:text-[#D35400]"
        }`
      }
    >
      {icon}
